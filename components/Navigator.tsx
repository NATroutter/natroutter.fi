	...
	},
	{
		...
		dropdown: [
			...
			},
			{
				name: "List",
				description:
					"Everything I've watched, what I'm watching now, and my watchlist",
				url: "/anime/list",
				icon: IoIosListBox,
			},
			{
...
export default function Navigator() {
	...
	return (
		<div
			className={
				"flex justify-end md:justify-center bg-background shadow-nav relative"
			}
		>
			{/* Desktop Navigation */}
			<NavigationMenu
				...
				<NavigationMenuList className="gap-0">
					{navigatorData.map((item) =>
						item.dropdown ? (
							<NavigationMenuItem
								key={item.data.name}
								className="relative border-r-2 border-header first:border-l-2"
							>
							...
							<NavigationMenuItem
								key={item.data.name}
								className="border-r-2 border-header first:border-l-2 z-40"
							>
								<NavigationMenuLink
									href={item.data.url}
